...
export default function HUD() {
  ...
  return (
    <div className="bg-slate-950/80 text-slate-100 p-4 border-b border-sky-500/20 backdrop-blur-md">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 text-xs">
        ...
        <div className="p-3 bg-slate-900/60 rounded border border-slate-800">
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold uppercase text-sky-400 flex items-center gap-1.5">
          ...
          <div className="space-y-1">
            <div className="flex justify-between">
              ...
              <span className={worldState.corruption_level > 20 ? 'text-amber-400 font-bold' : 'text-emerald-400'}>
                {worldState.corruption_level}%
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Prism Harmonic:</span>
              <span className="text-sky-300 font-mono">{worldState.prism_harmonic}%</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="p-3 bg-slate-900/60 rounded border border-slate-800">
          <div className="flex items-center gap-1.5 mb-2 font-semibold uppercase text-sky-400">
          ...
          <div className="space-y-1">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-amber-300">Lvl {player.level} ({player.xp} XP)</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Dermal Nodes:</span>
              <span className="text-emerald-400">{player.biometrics?.dermalNodes || 'Calibrated'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-3 bg-slate-900/60 rounded border border-slate-800">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="px-2 py-0.5 bg-sky-600/30 text-sky-300 rounded border border-sky-500/40 hover:bg-sky-600/50"
            >
              Generate
            </button>
          </div>
          {missions.length > 0 ? (
            <div className="space-y-1">
              ...
              <div className="text-amber-300 text-[10px]">Reward: +{missions[0].rewards?.xp} XP, {missions[0].rewards?.credits} Cred</div>
            </div>
          ) : (
        ...
        <div className="p-3 bg-slate-900/60 rounded border border-slate-800 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <button
              ...
              className="px-2 py-1 bg-amber-900/40 border border-amber-600/40 text-amber-300 rounded hover:bg-amber-800/50 text-[11px] text-center"
            >
              Cast Fire Rune
            </button>
            <button
              onClick={() => requestNPCDialogue('Mai', 'Provide status update on PRISM harmonic')}
              className="px-2 py-1 bg-indigo-900/40 border border-indigo-600/40 text-indigo-300 rounded hover:bg-indigo-800/50 text-[11px] text-center"
            >
              Ping AI (Mai)
            </button>
          </div>
